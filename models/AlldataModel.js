...
class AlldataModel {
    ...
    */
    static async queryList(data) {
        ...
        let {Founder, time, Audit, Entrance} = searchVal

        let totalSQL = 'SELECT * FROM (SELECT * FROM standard union all SELECT * FROM integral UNION all SELECT * FROM alldata) as table_zjz where queryKey=1'

        let dataSQL = `SELECT * FROM (SELECT * FROM standard union all SELECT * FROM integral UNION all SELECT * FROM alldata) as table_zjz where queryKey=1`
        ...
        //查询申报窗口
        if(Entrance){
            totalSQL += ` AND Entrance='${Entrance}'`

            dataSQL += ` AND Entrance='${Entrance}'`
        }
        //筛选时间查询
        if(time){
